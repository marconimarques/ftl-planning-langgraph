...
@dataclass
class CoverageComparisonResult:
    """Result wrapper for a two-level coverage comparison (two solver runs).

    Created by run_coverage_comparison. Callers can use isinstance(result,
    CoverageComparisonResult) instead of checking cost_a is not None.
    """
    result_a: MILPResult
    result_b: MILPResult
    coverage_count_a: int
    coverage_count_b: int

    @property
    def feasible(self) -> bool:
        return self.result_a.feasible and self.result_b.feasible

    @property
    def cost_difference(self) -> float:
        return self.result_b.total_cost - self.result_a.total_cost

    @property
    def trucks_a(self) -> int:
        return self.result_a.trucks

    @property
    def trucks_b(self) -> int:
        return self.result_b.trucks

    @property
    def cost_a(self) -> float:
        return self.result_a.total_cost

    @property
    def cost_b(self) -> float:
        return self.result_b.total_cost

    def as_milp_result(self) -> MILPResult:
        """Return result_b with the two-level fields populated for MILPResult-only consumers.

        `trucks_a/b`, `cost_a/b`, and `cost_difference` only exist as computed
        properties here; callers that pass result_b onward as a plain MILPResult
        (e.g. consistency checks, PipelineResult.milp_result) need them copied
        onto the object itself or those fields read back as None.
        """
        self.result_b.trucks_a = self.trucks_a
        self.result_b.trucks_b = self.trucks_b
        self.result_b.cost_a = self.cost_a
        self.result_b.cost_b = self.cost_b
        self.result_b.cost_difference = self.cost_difference
        self.result_b.coverage_count_a = self.coverage_count_a
        self.result_b.coverage_count_b = self.coverage_count_b
