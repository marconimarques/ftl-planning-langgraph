...
def _rank_improving_strategies(strategies: list[StrategyResult]) -> list[StrategyResult]:
    return sorted(
        [strategy for strategy in strategies if _is_improving_strategy(strategy)],
        key=lambda item: item.cost_recovered,
        reverse=True,
    )
...
def _fallback_strategy_changes(network: NetworkData, shock_params: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for field in ("availability", "overtime_hours", "payload", "speed_loaded", "speed_empty", "working_days"):
        change = _operational_candidate(network, shock_params, field)
        if change:
    ...
    if operational and variable_savings:
        candidates.append({**operational[0], **variable_savings[0]})
    if operational and fixed_savings:
        candidates.append({**operational[0], **fixed_savings[0]})
...
def solve_shock_candidate_plan(
    ...
    for candidate_changes in _fallback_strategy_changes(network, shock_params_dict):
        if len(_rank_improving_strategies(solved_strategies)) >= target_strategy_count:
            break
        signature = _change_signature(candidate_changes)
        if signature in seen_changes:
            continue
    ...
    narrative = _build_deterministic_shock_narrative(
        _rank_improving_strategies(solved_strategies),
        (
    ...
    return ShockResponseOutput(
        ...
        strategies=_rank_improving_strategies(solved_strategies)[:target_strategy_count],
        narrative=narrative,
    )
