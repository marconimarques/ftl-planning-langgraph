...
_SCALAR_SHOCK_FIELDS = (
    "payload", "speed_loaded", "speed_empty", "availability",
    "overtime_hours", "overtime_cost", "working_days", "net_driving_hours",
)


def _conflicts_with_shock(shock_params: dict[str, Any] | None, changes: dict[str, Any]) -> bool:
    """True if a candidate re-touches the exact requirement the shock already set.

    Applies to every shock field, not just cost components: a strategy cannot
    simultaneously model "fuel +5%" (the shock) and "fuel -5%" (a compensating
    saving) on the same component, and it cannot propose a new "payload" value
    when the shock itself is a payload degradation — since `combined_changes =
    {**shock_params, **candidate_changes}` is a dict merge, reusing the same key
    would silently overwrite the shock's degraded value instead of compensating
    for it. Either way, that isn't a real response — it's re-answering the same
    requirement the query already stated.
    """
    if not shock_params:
        return False
    for field in _SCALAR_SHOCK_FIELDS:
        if field in shock_params and field in changes:
            return True
    for field in ("var_cost_multipliers", "fix_cost_multipliers"):
        shocked_keys = _shocked_cost_keys(shock_params, field)
        if not shocked_keys:
            continue
        for key in _mapping_or_empty(changes.get(field)):
            if key in shocked_keys or _norm_key(key) in shocked_keys:
                return True
    return False


def _changes_are_valid_response_levers(
    network: NetworkData,
    changes: dict[str, Any],
    ...
    if any(field in changes for field in _NON_RESPONSE_FIELDS):
        return False
    if _conflicts_with_shock(shock_params, changes):
        return False
...
def _fallback_strategy_changes(network: NetworkData, shock_params: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for change in candidates:
        ...
        if signature not in seen and _changes_are_valid_response_levers(network, change, shock_params):
            seen.add(signature)
...
def _solve_candidate_changes(
    ...
    if not _changes_are_valid_response_levers(network, candidate_changes, shock_params_dict):
        return None
