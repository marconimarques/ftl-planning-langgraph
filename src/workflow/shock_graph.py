...
def validate_and_rank_shock_output(output):
    """Validate numeric shock strategy fields and rank strategies in Python."""
    ...
    ranked = sorted(
        ...
        key=lambda item: getattr(item, "cost_recovered", 0.0),
        reverse=True,
    )
    try:
...
def build_shock_graph(
    *,
    ...
    def _validate_and_rank(state):
        shock_output = validate_and_rank_shock_output(state["shock_output"])
        return {
